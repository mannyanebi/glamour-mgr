function setMaleDressDay3(weatherSummary, summaryInfo) {
    if (weatherSummary) {
    ...
    document.querySelector("#male-dress-day3").appendChild(male_dress_img);    

    //for summary information gotten from server
    document.querySelector('#summary-info-day3').innerHTML = summaryInfo;
    } else {
        //remove male dress div so that nothing will display since
        //no data was received from the server
        document.querySelector("#male-dress-day3").setAttribute('style', 'display:none');
        document.querySelector('#summary-info-day3').setAttribute('style', 'display:none');
    }
}