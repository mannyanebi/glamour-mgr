        ...
        .then(function (response) {
            ...
            .then(function (weather_responses) {
                ...
                for (let i = 0; i < getRequestTracker.length; i++) {
                    weather_info.push(weather_responses[i].data.daily.data[0].icon);
                }

                //this collects all the respective weather summary for each day and puts them into 
                //weather_summary array
                let weather_summary_info = [];
                for (let i = 0; i < getRequestTracker.length; i++) {
                    weather_summary_info.push(weather_responses[0].data.daily.data[0].summary);
                }

                //sends this response to the back to the client
                res.send({
                    "weather_summary_info": weather_summary_info,
                    "weather_info": weather_info
                });
                //prints the weather_info arrays
                console.log(JSON.stringify(weather_info, null, 2));
                console.log(JSON.stringify(weather_summary_info, null, 2));
             })
             .catch(function (err) {
                 res.send(err);
             });
